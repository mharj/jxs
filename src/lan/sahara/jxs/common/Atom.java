 ...
 */
public class Atom {
	private static final String[] _predefinedAtoms = { "PRIMARY", "SECONDARY", "ARC", "ATOM", "BITMAP", "CARDINAL", "COLORMAP", "CURSOR", "CUT_BUFFER0", "CUT_BUFFER1", "CUT_BUFFER2", "CUT_BUFFER3", "CUT_BUFFER4", "CUT_BUFFER5", "CUT_BUFFER6", "CUT_BUFFER7", "DRAWABLE", "FONT", "INTEGER", "PIXMAP", "POINT", "RECTANGLE", "RESOURCE_MANAGER", "RGB_COLOR_MAP", "RGB_BEST_MAP", "RGB_BLUE_MAP", "RGB_DEFAULT_MAP", "RGB_GRAY_MAP", "RGB_GREEN_MAP", "RGB_RED_MAP", "STRING", "VISUALID", "WINDOW", "WM_COMMAND", "WM_HINTS", "WM_CLIENT_MACHINE", "WM_ICON_NAME", "WM_ICON_SIZE", "WM_NAME", "WM_NORMAL_HINTS", "WM_SIZE_HINTS", "WM_ZOOM_HINTS", "MIN_SPACE", "NORM_SPACE", "MAX_SPACE", "END_SPACE", "SUPERSCRIPT_X", "SUPERSCRIPT_Y", "SUBSCRIPT_X", "SUBSCRIPT_Y", "UNDERLINE_POSITION", "UNDERLINE_THICKNESS",
			"STRIKEOUT_ASCENT", "STRIKEOUT_DESCENT", "ITALIC_ANGLE", "X_HEIGHT", "QUAD_WIDTH", "WEIGHT", "POINT_SIZE", "RESOLUTION", "COPYRIGHT", "NOTICE", "FONT_NAME", "FAMILY_NAME", "FULL_NAME", "CAP_HEIGHT", "WM_CLASS", "WM_TRANSIENT_FOR" };
	 ...
	 * @param xServer
	 */
	public static void registerPredefinedAtoms(AbsApiServer apiServer) {
		for (int i = 0; i < _predefinedAtoms.length; i++)
			apiServer.addAtom(new Atom(i + 1, _predefinedAtoms[i]));
	}
	 ...
	 *            Bytes yet to be read in the request.
	 * @throws IOException
	 */
	public static void processGetAtomNameRequest(AbsApiServer apiServer, Client client, int bytesRemaining) throws IOException {
	...
	/**
	 * Process an InternAtom request. Return or create an atom with the
	 * specified name.
	 * 
	 * @param xServer
	 *            The X server.
	 ...
	 * @throws IOException
	 */
	public static void processInternAtomRequest(AbsApiServer apiServer, Client client, byte arg, int bytesRemaining) throws IOException {
		InputOutput io = client.getInputOutput();
		...
		Atom a = apiServer.findAtom(s);

		if (a != null) {
			id = a.getId();
		} else if (!onlyIfExists) {
			a = new Atom(apiServer.nextFreeAtomId(), s);
			apiServer.addAtom(a);
			id = a.getId();
		}
